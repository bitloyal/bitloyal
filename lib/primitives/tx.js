...
TX.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.time != null) {
    assert(util.isU32(options.time), 'Time must be a uint32.');
...
TX.prototype.checkInputs = function checkInputs(view, height) {
  ...
  let stakeAmount = 1800000000; // TODO FIXME only if staking

  // TODO if isStake + block height stake rate
  if (total + stakeAmount < value)
    return [-1, 'bad-txns-in-belowout', 100];

  const fee = total - value + stakeAmount;
...
TX.prototype.fromJSON = function fromJSON(json) {
  ...
  assert(util.isU32(json.time), 'Time must be a uint32.');
...
/**
 * Determine if this is staking.
 * @returns {boolean}
 */

TX.prototype.isStaking = function isStaking() {
  for (const input of this.inputs) {
    if (input.prevout !== null && input.prevout.hash === encoding.NULL_HASH) {
      return true;
    }
  }
